  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        if (!user) {
          setError('Authentication required to view telemetry.')
          setLoading(false)
          return
        }

        const eventsRef = collection(db, `users/${user.uid}/telemetry`)
        const eventsQuery = query(eventsRef, orderBy('createdAt', 'desc'), limit(1000))

        unsubscribe = onSnapshot(eventsQuery, (snap) => {
          ...
          setEvents(next.filter((event) => event.eventName))
          setLoading(false)
        }, (snapshotError) => {
          console.error('[Telemetry] Failed to load events:', snapshotError)
          setError('Unable to load telemetry data right now.')
          setLoading(false)
        })
      } catch (loadError) {
        console.error('[Telemetry] Setup error:', loadError)
        setError('Unable to load telemetry data right now.')
        setLoading(false)
      }
    }
