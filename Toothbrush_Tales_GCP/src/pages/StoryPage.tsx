...
export default function StoryPage() {
  ...
  const audioQueueRef = useRef<{ text: string; voiceId: string; audioUrlPromise?: Promise<string | null> }[]>([])
  const isProcessingQueueRef = useRef(false)
  const segmentAudioPromisesRef = useRef<(Promise<string | null> | null)[]>([null, null, null, null])
  const conclusionAudioPromiseRef = useRef<Promise<string | null> | null>(null)
  ...
  // Same as queueSpeech but reuses an already-in-flight synthesize() promise
  // so segment audio is ready (or close to it) by the time its slot starts.
  const queuePrefetched = useCallback((text: string, voiceIdToUse: string, audioUrlPromise: Promise<string | null> | null) => {
    audioQueueRef.current.push({
      text,
      voiceId: voiceIdToUse,
      audioUrlPromise: audioUrlPromise ?? undefined,
    })
    processQueue()
  }, [processQueue])
  ...
  // Auto-play TTS when segment changes during brushing phase. Skips the
  // spoken brushing-prompt; the on-screen segment label covers it visually.
  useEffect(() => {
    ...
    if (
      ...
      if (segmentText) {
        queuePrefetched(combinedText, voiceId, segmentAudioPromisesRef.current[currentSegment])
        setSpokenSegments(prev => new Set(prev).add(currentSegment))
      }
    }
  }, [currentSegment, phase, autoPlay, playbackMode, queuePrefetched, spokenSegments, story.segments, story.brushingPrompts, voiceId])

  // Queue conclusion when complete
  useEffect(() => {
    if (phase === 'complete' && autoPlay && (playbackMode === 'audio' || playbackMode === 'both')) {
      queuePrefetched(story.conclusion, voiceId, conclusionAudioPromiseRef.current)
    }
  }, [phase, autoPlay, playbackMode, queuePrefetched, story.conclusion, voiceId])
