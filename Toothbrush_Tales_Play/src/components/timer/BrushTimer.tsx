...
export default function BrushTimer({
  ...
  // Stash callbacks in refs so the RAF effect doesn't tear down on every parent rerender.
  const onCompleteRef = useRef(onComplete)
  const onSegmentChangeRef = useRef(onSegmentChange)
  const lastNotifiedSegmentRef = useRef<number | null>(null)
  ...
  useEffect(() => {
    ...
    lastNotifiedSegmentRef.current = null
  ...
  // Notify parent on actual segment transitions only (not every render).
  useEffect(() => {
    if (timeRemaining <= 0) return
    if (lastNotifiedSegmentRef.current === currentSegment) return
    lastNotifiedSegmentRef.current = currentSegment
    onSegmentChangeRef.current(currentSegment)
  }, [currentSegment, timeRemaining])
