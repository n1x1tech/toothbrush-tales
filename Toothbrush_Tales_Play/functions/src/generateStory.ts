...
// Firestore TTL: documents are auto-deleted ~30 days after this timestamp.
// The TTL policy on the `expiresAt` field must be enabled in Firebase Console
// (Firestore → Time-to-live → add policy on storyRequests/expiresAt).
const TTL_DAYS = 30
const computeExpiresAt = () => new Date(Date.now() + TTL_DAYS * 24 * 60 * 60 * 1000)
  ...
  .onCreate(async (snap) => {
    try {
      ...
      if (!characterName || !theme) {
        await snap.ref.update({ status: 'error', error: 'characterName and theme are required', expiresAt: computeExpiresAt() })
        return
      }
      ...
      try {
        await snap.ref.update({ status: 'error', error: 'Internal error', expiresAt: computeExpiresAt() })
